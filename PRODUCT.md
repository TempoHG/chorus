...
Delegated: Astro, deployed as a static-first marketing site. Rationale: this is a lead-gen landing page (not an app) that needs to embed third-party GoHighLevel widgets (forms, booking calendar) and load fast for SEO/ad-traffic conversion; Astro ships minimal JS by default, keeps the page content-driven, and makes dropping in GHL script embeds straightforward. Revisit if the page grows interactive requirements beyond embeds.
...
- Chorus — $397/mo. One product, complete on its own: website & lead capture, Restaurant Systems, unified guest inbox, guest database & marketing, reviews & reputation, automations & reporting, social planning, white-glove onboarding, training and support. There is no second or third tier — never present Chorus as having multiple plans/tiers to choose between.
- Onboarding: a $497 one-time fee, waived in full for restaurants that start with a 3-month commitment. Committed customers pay $299/month during and after that commitment (Chorus continues month-to-month once the 3 months are up) — this is the one place a lower recurring price than $397/mo is correct, and only in the context of the 3-month-commitment offer.
- Multi-location: a flat +$149/mo for each additional location. One rate, full stop — there is no per-tier variation to reference.
- Add-ons: not currently shown on the public site (removed from both the pricing and features pages when the onboarding-fee pricing was introduced). AI Restaurant Assistant, AI Phone, AI Web & Text Conversations and AI Review Responses are no longer separate add-ons — they're merged into Chorus's included AI capabilities (see the pricing page's Stay Connected / Bring Guests Back / Save Time groups). The remaining true add-ons, if this section returns to the site: Local Presence, Local SEO, AI Unlimited, Dedicated Email, WhatsApp — still never priced; the pricing architecture for these is being reconsidered post-simplification, so do not invent figures.
